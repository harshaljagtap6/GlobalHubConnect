...
//_________define class_________________________// 
	class DbClass {	
		...
		function saveData($insData,$tableName){ 
	        ...
	        foreach($insData as $key => $value){
	            $fileds[] = "`{$key}`";
	            if(!is_numeric($value)){
	                $field_val = mysqli_real_escape_string($value);
	                $values[] = "'{$field_val}'";
	            }else{
	                $values[] = $value;
	            }
		...
		function fetchRow($sql="")	{	
			...
			{
		  	    $data  = mysqli_fetch_array($results,MYSQL_ASSOC);
				mysqli_free_result($results);
				return $data;
		 	}
		}	
		...
		*/
		function tableExists($tablename)
		{
			...
			$results = mysqli_list_tables(DB_DATABASE) or die("Could not access Table List...<hr>" . mysql_error());
	...
	function setQuery($sql=""){
		...
		$conn = $this->CONN;
		$results = mysqli_query($sql,$conn) or $this->error("Something went wrong in the query<hr>$sql<hr>");
		if(!$results){
			$this->error("Query went bad ! <hr>$sql<hr>");
			return false;	
		}else{
	...
	function dbFieldName($result) {
       return mysqli_field_name($result);
	}
	
	function dbFieldType($result) {
       return mysqli_field_type($result);
	}
	...
	function dbTablesName(){
		$data = array();
		$tablesname = mysqli_list_tables(DB_DATABASE);
	
		$i=0;
		while ($row = mysqli_fetch_array($tablesname)){
			$data[$i] = $row[0];
			$i++;
		}
